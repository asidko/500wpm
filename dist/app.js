...
function tokenizeText(text) {
  ...
  text = text.replace(urlPattern, function (match) {
    var placeholder = "__URL_".concat(placeholderIndex, "__");
    placeholders.push({
  ...
  text = text.replace(emailPattern, function (match) {
    var placeholder = "__EMAIL_".concat(placeholderIndex, "__");
    placeholders.push({
      placeholder: placeholder,
      original: match
    });
  ...
  text = text.replace(phonePattern, function (match) {
    var placeholder = "__PHONE_".concat(placeholderIndex, "__");
    placeholders.push({
      placeholder: placeholder,
      original: match
    });
  ...
  text = text.replace(numberPattern, function (match) {
    var placeholder = "__NUMBER_".concat(placeholderIndex, "__");
    placeholders.push({
      placeholder: placeholder,
      original: match
    });
...
function initHomePage() {
  ...
  for (var i = 0; i < modeTabs.length; i++) {
    addEvent(modeTabs[i], 'click', function () {
      ...
      // Update input areas
      for (var _j = 0; _j < inputModes.length; _j++) {
        inputModes[_j].className = 'input-mode';
      }
...
function initReaderPage() {
  ...
  addEvent(increaseSpeed, 'click', function () {
    adjustSpeed(50);
  });
  addEvent(decreaseSpeed, 'click', function () {
    adjustSpeed(-50);
  });
  ...
  addEvent(forwardButton, 'click', function () {
    skipWords(10);
  });
  ...
  function skipWords(count) {
    ...
    state.currentWordIndex = Math.max(0, Math.min(state.allWords.length - 1, state.currentWordIndex + count));
...
function applyColorScheme(isDark) {
  ...
  var elements = {
    ...
    '.file-button, .start-button, .speed-button, .control-button': {
      bg: isDark ? '#fff' : '#000',
      color: isDark ? '#000' : '#fff'
    },
    '#wordDisplay, .app-title': {
      color: isDark ? '#e0e0e0' : '#000'
    },
    '.reader-controls, .bottom-controls': {
      bg: isDark ? 'rgba(26, 26, 26, 0.95)' : 'rgba(255, 255, 255, 0.95)',
      border: isDark ? '#444' : '#ddd'
    },
    '.paused-indicator, .progress-display': {
      color: isDark ? '#999' : '#666'
    },
    '.chapter-list': {
      bg: isDark ? '#2a2a2a' : '#f9f9f9',
      border: isDark ? '#444' : '#ddd'
    },
    '.chapter-item': {
      bg: isDark ? '#333' : '#fff',
